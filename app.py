# filepath: /c:/Users/saqib/OneDrive - McMaster University/Term_2_Soft-eng/saqib_delta/deltahacks/app.py
...
def shoulder_press_gen():
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            cv2.putText(frame, 'Presses: {}'.format(press_count), (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            mp_drawing.draw_landmarks(frame, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)
            ret, buffer = cv2.imencode('.jpg', frame)
            frame = buffer.tobytes()
...
def lateral_raise_gen():
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            cv2.putText(frame, 'Raises: {}'.format(raise_count), (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            mp_drawing.draw_landmarks(frame, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)
            ret, buffer = cv2.imencode('.jpg', frame)
            frame = buffer.tobytes()
...
def shoulder_press_feed():
    return Response(shoulder_press_gen(),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
...
def lateral_raise_feed():
    return Response(lateral_raise_gen(),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
